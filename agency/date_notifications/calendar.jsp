...
%>
	...
	<%
		...
		db.connect();
		ResultSet rs= db.execSQL(sql_count);
		if(rs.next()) 
		{
		rows= rs.getString(1);
		}
		...
		String[][] db_events = new String[1][8];


		String sql_schedule="select * from schedule ;" ;
		db.connect();
		ResultSet res= db.execSQL(sql_schedule);
		int i=0;
		while(res.next()) 
		{

		db_events[i][0]=res.getString(2);
		db_events[i][1]=res.getString(3);
		db_events[i][2]=res.getString(4);
		db_events[i][3]=res.getString(5);
		db_events[i][4]=res.getString(6);
		db_events[i][5]=res.getString(7);
		db_events[i][6]=res.getString(8);
		db_events[i][7]=res.getString(9);
		i++;
out.println(i);
		}
		res.close();
...
events[0][0]="<%=db_events[0][0]%>";
events[0][1]="<%=db_events[0][1]%>";
events[0][2]="<%=db_events[0][2]%>";
events[0][3]="<%=db_events[0][3]%>";
events[0][4]="<%=db_events[0][4]%>";
events[0][5]="<%=db_events[0][5]%>";
events[0][6]="<%=db_events[0][6]%>";
events[0][7]="<%=db_events[0][7]%>";
...
<table id="evtcal" border="0" cellpadding="0" cellspacing="0" width="584">
	<tbody>
		<tr>
			...
			<td width="10">&nbsp;</td>
			<td style="padding: 3px;" align="center" bgcolor="#ffffc8" valign="top" width="260"><b>Intructions:</b><br />			  
      Click a highlighted date on the calendar to see the schedule of companies visting.<br /><br />
                ...
                <input type="button" onclick='arrays();' value="Click me "/>
				</center>
			</td>
		</tr>
	</tbody>
...
<p style="font: normal 11px Arial" align="center">&nbsp;</p>
