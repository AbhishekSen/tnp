...
<form name="form_query_placement" id="form_query_placement">
  <%        
		...
		//ResultSet re=db.execSQL("select schedule.company_name,schedule.day,schedule.month,schedule.year,schedule.starting_time,schedule.end_time from schedule;");
		
		ResultSet re=db.execSQL("select * from schedule order by agency_id");// schedule.company_name,schedule.day,schedule.month,schedule.year,schedule.starting_time,schedule.end_time from schedule;");
		out.println("<table align=\"center\" border=\"1\"><b><tr><td>S.No.</td><td>Organization Name</td><td>Profile Name</td><td>Date From</td><td>Date To</td><td>Venue</td></tr></b>");
		while(re.next())
 		{
                    int ag_id=re.getInt("agency_id");
                    ResultSet rs_get=db.execSQL("select agency_name from agency where agency_id="+ag_id+";");
                    rs_get.next();
                    String ag_name=rs_get.getString("agency_name");   
                    rs_get.close();

	            out.println("<tr><td>"+co+"</td><td>"+ag_name+"</td><td>"+re.getString("company_profile")+"</td><td>"+re.getString("date_from")+", "+re.getString("starting_time")+"</td><td>"+re.getString("date_to")+", "+re.getString("end_time")+"</td><td>"+re.getString("venue")+"</td></tr>");
		    co++;
		}
		out.println("</table>");
		re.close();
