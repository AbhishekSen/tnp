 ...
 <%
	...
	try{
		...
		/**********************/
			...
			/***********************************/
			//If package was approved previously
			/***********************************/
			if(!old_package_id.equals("0")){
				...
				while(rss.next()){
					...
					if(rss.getInt("program_index")<=11){
						...
						String qry = "insert into eligible_students (agency_id,student_id,package_id)"
						+" (select M.agency_id,A.student_id, M.package_id"
						+" from minor_details M,"
						+" (select student_id, student_record.program_id as major, minor_department.department_id as minor,"
						+" student_record.cpi_6sem as cpi"
						+" from student_minor_details,"
						+" minor_department, student_record"
						+" where student_minor_details.student_roll_number = student_record.student_roll_number"
						+" and student_minor_details.minor_program_name = minor_department.department_name"
						+" ) A where "
						+" A.major = M.major "
						+" and A.minor = M.minor and A.cpi >= M.cpi"
						+" and M.agency_id = "+ agency_id
						+" and M.package_id = "+ old_package_id
						+" and (M.agency_id,A.student_id, M.package_id)"
						+" not in (select agency_id,student_id,package_id from eligible_students));";

						int sqlMinor = db.updateSQL(qry);	
						
						
					}					else if( rss.getInt("program_index")<31  || (rss.getInt("program_index")>=45 && rss.getInt("program_index")<=49 )) {
			...
			/**************************************************/
			//If package is freshly added ( first time approval) 
			/***************************************************/
			else{
				...
				//Inserting a new package in package_details from package_details_temp ( new package_id is auto generated ).
				...
				ag_id=0;
				if(rrs.next()){
					// ag_id is the autogenerated package_id of package_details table
					ag_id=rrs.getInt(1);
				}
				rrs.close();
				...
				// Deleting previously applied students (if any.. should not be there generally )
				...
				// Getting data frm package_extra_details_temp
				ResultSet rss=db.execSQL("select * from package_extra_details_temp where package_id="+pack_id+";");
				while(rss.next()){
				
					// Inserting into package_extra_details
					...
					// Program Index available in prog_record
					
					//Program_index <11 denoting bachelors ( btech or bdes ).
					if(rss.getInt("program_index")<=11){
					...
					//For Masters Program Index 12-30 and 45-50
					else if(rss.getInt("program_index")<31 || (rss.getInt("program_index")>=45 && rss.getInt("program_index")<=49 )){
