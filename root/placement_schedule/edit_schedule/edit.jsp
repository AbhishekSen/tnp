...
{	
	...
	if(message=="")
	{
		document.form_placement_record.action="edit.jsp";
		return true;
	}
	else
	{
	 ...
	 if(submit!=null && agency_id!=null )
          {
            String sql_count="select count(*) from schedule where agency_id="+Integer.parseInt(agency_id)+";";
             ...
             if(num1>0)
             {
                  //response.sendRedirect("update.jsp?company_name="+agency_id); 
                  response.sendRedirect("check.jsp?company_name="+agency_id); 
             }
             //else
		//response.sendRedirect("check.jsp?company_name="+agency_id);
          }
...
<%
  db.connect(); 
  ResultSet rs_agency_lst=db.execSQL("select distinct  sc.company_profile,sc.agency_id,a.agency_name from schedule as sc,agency as a where sc.agency_id=a.agency_id order by sc.agency_id;"); 
  %>
  ...
  <% 
    ...
    db.close();
  // agency_list =drop_down("select company_profile from schedule  ;",db) ; 
  %>
</select>	
</td></tr>
