...
#evtcal a:active {font: normal 12pt "Arial", "Helvetica", "Sans Serif"; color: #004400; text-decoration: none;}		/* selected link */
	...
	<%
		...
		//out.println(rows);
	       ...
	       String[] db_agencyid = new String[rows];
              String[] db_agencyname = new String[rows];
            
           if(submit!=null)
            {
              sql_agencyid+="select agency_id from interested_students where student_id=='"+student_id+"';" ;
              db.connect();
              reas5= db.execSQL(sql_agencyid);
               ...
               while(reas5.next()) 
		{ 
                  db_agencyid[num]=reas5.getString(1);
                      num++;
                }
              reas5.close();
              db.close();
               ...
               while(num<rows)
                {
                  db.connect();
                  sql_schedule="";
                  sql_schedule+="select agency_name from agency where agency_id='"+db_agencyid[num]+"';" ;
                  res5= db.execSQL(sql_schedule);
                  while(res5.next()) 
		  { 
                  db_agencyname[num]=res5.getString(1);
                  }
                  res5.close();
                  db.close();
                  num++;
                ...
                while(num<rows)
                 {
                    db.connect();
                    sql_selected="";
                    sql_selected+="select * from schedule where company_name='"+db_agencyname[num]+"';" ;
                    sr5= db.execSQL(sql_selected);
		
		while(sr5.next()) 
		{

		db_events[i5][0]=sr5.getString("recurring");
		db_events[i5][1]=sr5.getString("month");
		db_events[i5][2]=sr5.getString("day");
		db_events[i5][3]=sr5.getString("year");
		db_events[i5][4]=sr5.getString("starting_time");
		db_events[i5][5]=sr5.getString("end_time");
		db_events[i5][6]=sr5.getString("company_name");
		db_events[i5][7]=sr5.getString("description");
		db_events[i5][8]=sr5.getString("venue");
		db_events[i5][9]=sr5.getString("eligibility_criteria");
		db_events[i5][10]=sr5.getString("departments");//.replaceAll("\n",">>");
		i5++;               
		}
		sr5.close();
		db.close();
...
<tr>
                      ...
                      <td><input name="org" type="radio" id="org" value="1" >
                        Selected Organisations     
                        <input type="radio" name="org" id="org" value="0" checked >
                        All Organisations<input type="submit" name="submit" value="Submit"></td>
                    </tr>  
   </table></form>
  <p>&nbsp;</p>
  <p>&nbsp;</p>
