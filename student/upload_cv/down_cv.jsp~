...
if(cv_id.equals("0") || cv_id.equals("1") )
{
	...
	db.connect();
		...
		else
		{
			...
			if(cv_id.equals("0"))
			        response.addHeader("Content-Disposition","attachment; filename="+student_roll_number+".pdf" );
			else if(cv_id.equals("1"))
         			response.addHeader("Content-Disposition","attachment; filename="+student_roll_number+"_1.pdf" );
